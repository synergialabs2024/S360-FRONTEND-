...
import { HandleSolGenericUnblockServicioModal } from '../../shared/components';
import { returnUrlSolicitudsDesbloqueoPrevetasPage } from './SolicitudsDesbloqueoPreventasMainPage';

export type SolicitudsDesbloqueoPreventaByStatePageProps = {
  state: GeneralModelStatesEnumChoice;
};
type MRTSServiceType = { row: MRT_Row<SolicitudDesbloqueoVentas> };

const SolicitudsDesbloqueoPreventaByStatePage: React.FC<
  SolicitudsDesbloqueoPreventaByStatePageProps
> = ({ state }) => {
  ...
  const columns = useMemo<MRT_ColumnDef<SolicitudDesbloqueoVentas>[]>(
    () => [
      ...
      {
        accessorKey: 'preventa__identificacion',
  ...
  return (
    <GridTableTabsContainerOnly>
      ...
      {/* ============== Modal =========== */}
      <HandleSolGenericUnblockServicioModal
        openModal={openModal}
        ...
        modelo={SalesModelsEnumChoice.PREVENTA}
        actionApprove={
          SalesStatesActionsEnumChoice.PREVENTA_SOLICITUD_DESBLOQUEO_APROBADO
        }
        actionReject={
          SalesStatesActionsEnumChoice.PREVENTA_SOLICITUD_DESBLOQUEO_RECHAZADO
        }
        modalTitle="Gestionar Solicitud Desbloqueo Preventa"
        firstTextModal="La solicitud de desbloqueo para la preventa "
        returnUrl={returnUrlSolicitudsDesbloqueoPrevetasPage}
      />
    </GridTableTabsContainerOnly>
  );
};

export default SolicitudsDesbloqueoPreventaByStatePage;
