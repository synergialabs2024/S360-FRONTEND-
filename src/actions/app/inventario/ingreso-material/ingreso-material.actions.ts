...
export const useCreateIngresoMaterial = <T>({
  navigate,
  returnUrl,
  returnErrorUrl,
  customMessageToast,
...
export const getIngresoMaterial = async (uuid: string) => {
  try {
    return await get<IngresoMaterial>(`/ingreso-material/${uuid}`, true);
  } catch (error) {
...
export const createIngresoMaterial = async <T>(
  ...
  return post<IngresoMaterial>('/ingreso-material/', data, true);
};
...
export const updateIngresoMaterial = async <T>({
  ...
  return patch<IngresoMaterial>(`/ingreso-material/${id}/`, data, true);
};
