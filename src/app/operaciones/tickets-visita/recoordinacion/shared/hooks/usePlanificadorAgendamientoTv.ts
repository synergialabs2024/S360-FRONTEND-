...
import { SaveFormDataAgendaTicketsVisita } from '../components/SaveRecoordinacionTicketVisita';
...
export const usePlanificadorAgendamientoTv = ({
  cackeKey,
  form,
  ...
  const calculateAvailableTimeSlots = (
    ...
    timeMapArray: TimeMapTicketVisitaPlanificador[],
    ...
    if (isToday) {
      ...
      if (lastInstaladoEntry) {
        lastInstaladoMinutes =
          timeStringToMinutes(lastInstaladoEntry.hora) + 60;
      }
      ...
      // Redondear adjustedStartMinutes al siguiente intervalo de 60 minutos
      adjustedStartMinutes = Math.ceil(adjustedStartMinutes / 60) * 60;
    ...
    // Generar los slots cada 60 minutos desde adjustedStartMinutes hasta endMinutes
    const hoursArray: string[] = [];
    for (let i = adjustedStartMinutes; i < endMinutes; i += 60) {
  ...
  useEffect(() => {
    (async () => {
      ...
      const timeMapArray: TimeMapTicketVisitaPlanificador[] =
        planificadores.flatMap(pl => pl.time_map || []);
      ...
      if (res?.status === HTTPResStatusCodeEnum.OK || res?.data) {
        ...
        startTimer(COUNTDOWN_TICKETS_VISITAS_ID, timerOtp, async () => {
