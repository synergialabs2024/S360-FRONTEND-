...
export type MonitoreosPageProps = {};
...
const MonitoreosPage: React.FC<MonitoreosPageProps> = () => {
  ...
  ///* mutations
  const changeState = useUpdateMonitoreo({
  ...
  ///* handlers
  const onEdit = (monitoreo: Monitoreo) => {
    setConfirmDialog({
      isOpen: true,
      ...
      onConfirm: () => {
        ...
        navigate(`${returnUrlMonitoreosPage}/editar/${monitoreo.uuid}`);
      },
    });
  };

  ///* columns
  const columns = useMemo<MRT_ColumnDef<Monitoreo>[]>(
    () => [
      {
        accessorKey: 'name',
        header: 'NOMBRE',
  ...
  return (
    ...
    >
      ...
      <CustomTable<Monitoreo>
        columns={columns}
        data={MonitoreosPagingRes?.data?.items || []}
        isLoading={isLoading}
        ...
        rowCount={MonitoreosPagingRes?.data?.meta?.count}
        // // actions
